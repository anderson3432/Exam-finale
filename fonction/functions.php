...
function get_produits_en_vente($id_categorie = null, $id_produit = null)
{
    $sql = "SELECT 
...
WHERE pm.quantite_dispo > 0";
    ...
    if (!empty($id_categorie)) {
        $sql .= " AND c.id_categorie = " . intval($id_categorie);
    }
    if (!empty($id_produit)) {
        $sql .= " AND p.id_produit = " . intval($id_produit);
    }

    $sql .= " ORDER BY pm.date_dispo DESC";
